    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      const oneShotScript =
        provider === "doubao"
          ...
          : provider === "yuanbao"
            ? `(function(){var imgs=Array.from(document.images).filter(function(img){if(!img||!img.src){return false;} var src=String(img.src); if(!/^https?:/i.test(src)){return false;} if((img.naturalWidth||0)<256||(img.naturalHeight||0)<256){return false;} var isYuanbaoCos=/hunyuan-prod-\\d+\\.cos\\.ap-guangzhou\\.myqcloud\\.com/i.test(src)||/\\.cos\\.[^/]+\\.myqcloud\\.com/i.test(src); return isYuanbaoCos;}); if(!imgs.length){throw new Error('image not found');} var src=imgs[imgs.length-1].src; return {ok:true, src:src, name:'${dynamicDownloadName}'};})()`
            : `(async function(){var imgs=Array.from(document.images).filter(function(img){if(!img||!img.src){return false;} var src=String(img.src); if(src.indexOf('id=file_')<0&&src.indexOf('/backend-api/')<0){return false;} var isLazy=String(img.loading||'').toLowerCase()==='lazy'; if(!isLazy){return false;} return true;}); if(!imgs.length){throw new Error('image not found');} var src=imgs[imgs.length-1].src; var r=await fetch(src,{credentials:'include'}); if(!r.ok){throw new Error('download failed: '+r.status);} var b=await r.blob(); var u=URL.createObjectURL(b); var a=document.createElement('a'); a.href=u; a.download='${dynamicDownloadName}'; document.body.appendChild(a); a.click(); setTimeout(function(){URL.revokeObjectURL(u); a.remove();},3000); return {ok:true, src:src, size:b.size, type:b.type, name:'${dynamicDownloadName}'};})()`;
